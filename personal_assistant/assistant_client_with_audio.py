#!/usr/bin/env python3
"""
Personal Assistant Client with Audio Playback
Handles both STT (microphone->server) and TTS playback (server->speaker)
"""
        ...
        try:
            async for message in self.websocket:
                try:
                    ...
                    # Handle binary data that came as string
                    if isinstance(message, bytes) and self.receiving_audio:
                        self.audio_buffer.extend(message)
                except Exception as e:
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] in ['-h', '--help']:
            print("Usage: python assistant_client_with_audio.py [server_url] [input_device] [output_device]")
            print("\nExamples:")
            print("  python assistant_client_with_audio.py")
            print("  python assistant_client_with_audio.py ws://192.168.1.100:9999")
            print("  python assistant_client_with_audio.py ws://localhost:9999 1 2")
            sys.exit(0)
