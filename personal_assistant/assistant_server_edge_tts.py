#!/usr/bin/env python3
"""
Personal Assistant Server with Anthropic Claude and Edge TTS
Direct Edge TTS integration for natural speech output
"""

if __name__ == '__main__':
    print("Starting Personal Assistant Server with Edge TTS...")
    import sys
    import os
    ...
    async def generate_edge_tts(text):
        """Generate TTS audio using Edge TTS and convert to PCM"""
        ...
        try:
            ...
            print(f">>> Converted to {len(pcm_data)} bytes of PCM audio")
    ...
    async def send_tts_audio(text):
        ...
        try:
            # Generate audio
            pcm_data = await generate_edge_tts(text)
            ...
            await send_to_clients(json.dumps({
                ...
                'channels': 1,
                'format': 'int16',
                'length': len(pcm_data) // 2,  # int16 samples
                'text': text
            }))
    ...
    async def main():
        ...
        try:
            ...
            test_audio = await generate_edge_tts("TTS system ready.")
            if test_audio:
        ...
        print(f"\n{'='*60}")
        print(f"Personal Assistant Server with Edge TTS")
        print(f"{'='*60}")
        ...
        print(f"  - Edge TTS audio streaming")
        print(f"    • Voice: {TTS_VOICE}")
        print(f"    • Natural speech synthesis")
